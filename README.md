...
### GUI (图形界面)
双击 `findex-ui.exe` 启动。启动后自动常驻系统托盘，无窗口显示。
...
### GUI
- 启动即系统托盘常驻，不显示窗口
- `Ctrl+Space` 呼出 egui 搜索浮窗（搜索框 + 实时结果列表）
- `Ctrl+Shift+F` 打开设置窗口（配置索引目录、搜索选项）
- 托盘右键菜单：显示设置 / 退出
...
│       ├── settings.rs      # Win32 原生设置窗口
│       ├── egui_win32.rs    # egui 软件光栅化渲染器
│       ├── egui_windows.rs  # egui 搜索浮窗
│       ├── config.rs        # 配置管理
│       └── main.rs          # 入口 & 热键 & 托盘
├── scripts/      # 构建工具脚本
...
- ✨ **文件图标美化** — 基于扩展名显示不同文件类型图标（代码/图片/文档/压缩包等）
...
- 🗑️ **删除旧 settings.rs** — Win32 原生设置窗口完全退役，所有 UI 统一为 egui
- 🔧 **热键统一** — Ctrl+Shift+F 和托盘菜单指向 egui 设置窗口
- ✅ **构建零警告**

### v0.1.5 (2026-07-28)
- 🔧 **修复: 字体纹理 API** — 锁定 Mutex 后再调用 size() 和 image()
- 🧹 **清理: 移除未使用代码** — 删除 ClippedPrimitive、Vertex 未使用导入
- 🗑️ **清理: 删除旧 search_overlay.rs** — 该模块已被 egui 搜索浮窗替代
- ✅ **构建零警告**
### v0.1.4 (2026-07-28)
